...
def makePNGs(root_folder, output_folder):
...
if __name__ == "__main__":
    ...
    makePNGs(input_folder, output_folder)

    print(F"Making pdf inside {pdf_output_folder}....")
    sh_file = "pngTopdf.sh"
    if not os.path.exists(join(output_folder, sh_file)):
        shutil.copyfile(sh_file, join(output_folder,sh_file))
    # Run it
    p = subprocess.Popen(["sh", join(output_folder,sh_file)], cwd=output_folder)
    p.wait()
