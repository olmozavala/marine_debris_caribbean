...
def sequential(start_date, end_date, config, name='', winds=True, diffusion=True, unbeaching=True, restart_file=""):
    ...
    base_folder = config[GlobalModel.vector_fields_folder]
    release_loc_folder = config[GlobalModel.location_folder]
    ...
    # main_fieldset = FieldSet.from_netcdf(file_names, variables, dimensions, allow_time_extrapolation=True, field_chunksize=(2048,2048))
    main_fieldset = FieldSet.from_netcdf(file_names, variables, dimensions, allow_time_extrapolation=True)
    ...
    #  If we use unbeaching we create a custom particle
    if unbeaching:
