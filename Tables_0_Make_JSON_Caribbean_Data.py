...
def readfrom(f):
    ...
    out_domain = float(all_lines[2].split(';')[0])
    out_domain_perc = float(remove_brackets(all_lines[2].split(';')[1]))

    at_beach = float(all_lines[3].split(';')[0])
    at_beach_perc = float(remove_brackets(all_lines[3].split(';')[1]))
            ...
            if cur_file.find("CBS") != -1:
                if cur_file.find("_from_") != -1:
                    ...
                    if not(country_name in all_names):
                        # Comment this part, only for debugging
...
if __name__ == "__main__":
    ...
    # output_folder = "/data/COAPS_Net/work/ozavala/CARIBBEAN_marine_debris/statistics/Caribbean"
    # makeJsonAndCsvFiles(input_folder, output_folder)
    # ----------------- Multiple years ---------------
    years = range(2017,2022)
    root_input_folder = "/nexsan/people/xbxu/HYCOM/GLBv0.08"
    root_output_folder = "/data/COAPS_Net/work/ozavala/CARIBBEAN_marine_debris/statistics/Caribbean/json"
    for year in years:
        input_folder = join(root_input_folder , F"C{year}_12")
        output_folder = join(root_output_folder, F"{year}")
        makeJsonAndCsvFiles(input_folder, output_folder)