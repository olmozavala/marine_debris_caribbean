...
do
  ...
  then
    ...
    cmd="mpirun -np ${proc} python GlobalModel.py ${c_start_date}:0 ${c_end_date}:0 True True False $run_name /data/UN_Litter_data/output/YesWinds_YesDiffusion_NoUnbeaching/YesWinds_YesDiffusion_NoUnbeaching_2010_MONTH.nc $inc_per_run"
  else
    # In this case it should start from the previous coordinates
    cmd="mpirun -np ${proc} python  GlobalModel.py ${c_start_date}:0 ${c_end_date}:0 True True False $run_name $output_path/${run_name}_${prev_start_date}_${prev_end_date}.nc $inc_per_run"
  fi
  echo $cmd
